  ...
  <view class="con">
    ...
    <view class="kc-info">
      ...
      <view class="cook-wrap">
        <view class="cook">
			    <view class="item"><image src="/static/images/cook/cook-icon-1.png" mode="aspectFit"></image><text>{{baiduData.cooking_time_info}}</text></view>
			    <view class="item"><image src="/static/images/cook/cook-icon-2.png" mode="aspectFit"></image><text>{{baiduData.tag_infos.flavor}}</text></view>
          <view class="item"><image src="/static/images/cook/cook-icon-3.png" mode="aspectFit"></image><text>{{baiduData.difficulty_info}}</text></view>
		    </view>
        <view class="cook cook-last">
			    <view class="item"><image src="/static/images/cook/cook-icon-4.png" mode="aspectFit"></image><text>{{baiduData.tag_infos.classify}}</text></view>
			    <view class="item"><image src="/static/images/cook/cook-icon-5.png" mode="aspectFit"></image><text>{{baiduData.tag_infos.cooking_method}}</text></view>
          <view class="item"><image src="/static/images/cook/cook-icon-6.png" mode="aspectFit"></image><text>{{baiduData.tag_infos.cuisine}}</text></view>
		    </view>
        <!-- <view class="cook">
		    	<li><img class="cook-icon" src="{$smarty.const.__STATIC_IMG__}/images/m5/Dish/cook/cook-icon-4.png" alt="分类" />{$data.baidu_supply_infos.tag_infos.classify}</li>
		    	<li><img class="cook-icon" src="{$smarty.const.__STATIC_IMG__}/images/m5/Dish/cook/cook-icon-5.png" alt="烹饪方法" />{$data.baidu_supply_infos.tag_infos.cooking_method}</li>
		    	<li><img class="cook-icon" src="{$smarty.const.__STATIC_IMG__}/images/m5/Dish/cook/cook-icon-6.png" alt="菜系" />{$data.baidu_supply_infos.tag_infos.cuisine}</li>
		    </view> -->
      </view>
      ...
      <block v-if="wouldList.list.length">
        ...
        <block v-else>
          ...
          <view class="zf-li" v-for="(item, index) in wouldList.list" :key="index">
            <image v-if="item.video.gif || item.img" :src="item.video.gif || item.img" class="zf-img" mode="aspectFill" @click="previewImage(item.img)"></image>
            <text class="zf-bz" v-if="!(!isOpenWould && index == 3)"><text class="zf-bz-num">{{item.num}}/</text><text class="num-total">{{wouldListTotal.length}}</text>{{item.info}}</text>
          </view>
    ...
    <view class="recomm-box" v-if="recipeList.length">
      <view class="recomm-title-box clearfix">
        ...
        <navigator class="more" :url="'/pages/search?code=' + model.name" @click="goMore" hover-class="none">查看全部</navigator>
      </view>
      <image :src="nyfData.img" @click="navTo('/pages/h5?path=' + nyfData.url,'','0')" class="nyf-banner" mode="aspectFit" v-if="nyfData.img"></image>
...
export default {
  data() {
    return {
      show: true,
      dishTipsBottom: 'dishTipsBottom',
      dishIngreBottom: 'dishIngreBottom',
      dishVideo: 'dishVideo',
      barType: 'dish',
      barText: '',
      scopeData: {
      ...
      showPop: false,
      // 支付弹窗
      vipFrom: '小程序-菜谱详情页播放器内的按钮',
      // 发起支付的地方
      shareData: '',
      // 分享数据
      shareText: '美食需要与家人、朋友共同分享',
      // 分享按钮的文案
      followData: '',
      // 关注数据
      completelyAgain: '',
  ...
  onLoad(options) {
    ...
    that.fromService = options.wType; 
  ...
  methods: {
    getModel() {
      ...
      request('/baidu/v1/Dish/topInfo', params).then(res => {
        if (res.code == 10000 && res.data) {
          ...
          this.tagInfo = this.baiduData.tag_infos;
          this.sorce = [];
          ...
          that.scopeData.dish_name = that.model.name;
         
          tip.loaded();
          that.pageInfo = res.data.pageInfo; // pageInfo.success = function() {
    ...
    goMore() {
      swan.reportAnalytics('more_relevant_btn', {
        resource: '菜谱详情页底部按钮'
      });
    },
    ...
    // error(event){
    //   console.log('贴片广告播放失败');
    //   console.log(event);
    // },
    goWelfare() {
      // utils.xhNavigateTo('/pages/h5?path=https://appweb.xiangha.com/baiduApp/xianghaClass&copy=香哈菜谱');
      swan.navigateToSmartProgram({
        appKey: 'fzYKkRyzHtDsLcbTf5pPIeEfMAfvRVfV',
        // 要打开的小程序 App Key
        path: 'pages/home/dashboard/index' // 打开的页面路径，如果为空则打开首
    ...
    goHome() {
      swan.reportAnalytics('homepage_btn', {
        resource: '普通菜谱页回首页按钮'
      });
      uni.switchTab({
        url: '/pages/home?'
    ...
    closeFollow() {
      let that = this;
      uni.setStorageSync('completelyAgain', false);
      that.completelyAgain = false;
    },

    goPlay() {
      let that = this;
      uni.setStorageSync('isWifi', 'true');
      that.isWifi = true;
    },

    /**
     *此处为单独的支付弹窗内关闭按钮回调；如果是因为广告拉起的支付；则会同时触发这个回调
     emit中尝试传参会报错
     */
    closeAd() {
    ...
    goSignIn(res) {
      ...
      that.isReturn = false;
      let jsCode = that.$parent.globalData.jsCode;
      let params = {
        jsCode: jsCode,
        nickName: userinfo.nickName,
        avatarUrl: userinfo.avatarUrl
      };
      request('/baidu/v1/Index/init', params).then(res => {
    ...
    onPageScroll(e) {
      ...
      if (that.scrollTop < e.scrollTop) {
        // 往上滑动
        if (e.scrollTop > 1000 && that.showRemindFirst) {
          that.showRemind = true;
          let time = setTimeout(() => {
            that.showRemind = false;
            that.showRemindFirst = false;
...
.recipe-detail-page {
  ...
  .con {
    ...
    .video-box {
      ...
      width: 100%;
      height: 422rpx;
      background: url("https://s2.cdn.xiangha.com/images/m5/Common/video-bg.png")
        no-repeat;
    ...
    .zf-box {
      ...
      .zf-li {
        ...
        .zf-img {
          ...
          width: 100%;
          height: 446rpx;
          background: url("http://s2.cdn.xiangha.com/images/m5/Common/step-bg.png")
            no-repeat;
          background-size: 100% 100%;
          border-radius: 8rpx;
          .dt-t {
